...
export default {
  ...
  methods: {
    ...
    changeCondition () {
      ...
      // 组装条件 params
      const params = {
        page: this.page.currentPage, // 如果条件改变 聚后悔的第一页
        per_page: this.page.pageSize,
        status: this.searchForm.status === 5 ? null : this.searchForm.status, // 5 是我们前端虚构的
        channel_id: this.searchForm.channel_id, // 就是表单数据
        begin_pubdate: this.searchForm.datePange && this.searchForm.datePange.length ? this.searchForm.dateRange[0] : null,
        end_pubdate: this.searchForm.datePange && this.searchForm.datePange.length > 1 ? this.searchForm.dateRange[1] : null
      }
      // 通过接口传入
